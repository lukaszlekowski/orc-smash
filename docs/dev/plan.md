...
- Gate: `pnpm typecheck && pnpm test`, plus env-gated `AGY_CONTRACT` before release sign-off.
...
- env-gated real-provider checks pass for all real providers touched by the current repo state (opencode, codex, claude, and agy) — if a provider cannot be exercised on the local machine, release remains blocked until its contract test runs on a credentialed runner, not merely "where credentials are available";
...
- **Deviations and Corrections**:
  ...
  - **agy auth hardening**: The detector is grounded in the operator-confirmed unauthenticated output `Error: authentication failed or timed out`, keeps generic `401`/`unauthorized` detection stderr-only, and strips fenced code before scanning. The env-gated `AGY_CONTRACT=1` real-provider proof remains a release-gate item.
...
- **Release-gate items still pending** (cannot run in this environment — require credentials / a
  ...
  - env-gated real-provider contract runs (`OPENCODE_CONTRACT=1 CODEX_CONTRACT=1 CLAUDE_CONTRACT=1
    AGY_CONTRACT=1 ... pnpm test`), including the new codex/claude/agy tiny-timeout proofs and the
    agy real spawn/lifecycle/timeout contract;
